"""
This is the version of rigidity used in the paper but it is currently too
dependent on proteingraph code.  Want to be able to create a function that acts
directly on the rigidity matrix.
"""
    ...
    def calculate_infrig(self, angles=True, dihedrals=True):
        ...
        while len(self.atom_list) != 0:
            ...
            for i, edge in enumerate(tet_edges):
                ...
                row = R_test[i]
                row[[3 * tet_dict[atom1_id], (3 * tet_dict[atom1_id]) + 1, (3 * tet_dict[atom1_id]) + 2]] = (
                            atom1_xyz - atom2_xyz)
                row[[3 * tet_dict[atom2_id], (3 * tet_dict[atom2_id]) + 1, (3 * tet_dict[atom2_id]) + 2]] = (
                            atom2_xyz - atom1_xyz)
            ...
            for i, inf_motion in enumerate(inf_motions):
                ...
                p0, p1, p2, p3 = (self.protein.atoms[tet[0]]).xyz, (self.protein.atoms[tet[1]]).xyz, (
                self.protein.atoms[tet[2]]).xyz, (self.protein.atoms[tet[3]]).xyz
                ...
                tet_indices = [atomid_to_index[tet[0]], atomid_to_index[tet[1]], atomid_to_index[tet[2]],
                               atomid_to_index[tet[3]]]
                ...
                inf_motion = np.array(np.split(inf_motion, len(inf_motion) / 3))[
                    np.array([atom.id for atom in self.atom_list])]
